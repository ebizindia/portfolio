var customergroupfuncs={
	...
	init: function(opt={}){
		$('.main-content').on(common_js_funcs.click_event,'td.clickable-cell',{self:customergroupfuncs},common_js_funcs.changeLocationWithDataProperty);
		$('.main-content').on(common_js_funcs.click_event,'.page-link',{self:customergroupfuncs},customergroupfuncs.changePage);
		$('.main-content').on(common_js_funcs.click_event,'.toggle-search',{self:customergroupfuncs},customergroupfuncs.toggleSearch);

		if(CAN_DELETE) // attach delete handler only if the user has the delete right
			$('.main-content').on(common_js_funcs.click_event,'.record-delete-button',{self:customergroupfuncs},customergroupfuncs.deleteCustomerGroup);

		$('#recs-list>thead>tr>th.sortable').bind(common_js_funcs.click_event,{self:customergroupfuncs},customergroupfuncs.sortTable);

		$('#rec_list_container').on(common_js_funcs.click_event,'.searched_elem .remove_filter',customergroupfuncs.clearSearch);

		$(window).hashchange(customergroupfuncs.onHashChange);
		$(window).hashchange();
	},
	
	toggleSearch: function(ev){
		let elem = $(ev.currentTarget);
		customergroupfuncs.setPanelVisibilityStatus('customergroup_search_toggle', elem.hasClass('search-form-visible')?'':'visible'); // set closed status for the search panel
		customergroupfuncs.showHidePanel('customergroup_search_toggle');
	},
	...
	showHidePanel: function(panel){
		if(panel === 'customergroup_search_toggle'){
			let show_srch_form = false;
			if (typeof(Storage) !== "undefined") {
				srch_frm_visible = localStorage.customergroup_search_toggle;
			} else {
				srch_frm_visible = Cookies.get('customergroup_search_toggle');
			}
	...
	showList:function(resp,otherparams){
		...
		$(".back-to-list-button").addClass('d-none').attr('href',"customer-groups.php#"+customergroupfuncs.curr_page_hash);
	...
	doSearch:function(){
		...
		customergroupfuncs.resetSearchParamsObj();
		let fld = '';
		$('.panel-search .srchfld').each(function(i, el){
			let val = $.trim($(el).val());
			if(val!=''){
				fld = $(el).data('fld');
				customergroupfuncs.setSearchParams({searchon:$(el).data('fld'),searchtype:$(el).data('type'),searchtext:val});
			}
		});

		if(customergroupfuncs.searchparams.length<=0)
			return false;

		var options={pno:1};
		customergroupfuncs.getList(options);
		return false;
	},
	...
	sortTable:function(e){
		...
		customergroupfuncs.sortparams=[];
		customergroupfuncs.sortparams.push({sorton:sorton, sortorder:sortorder});
		var options={pno:pno};
		customergroupfuncs.getList(options);
	...
	openRecordForEditing:function(recordid){
		...
		$("#common-processing-overlay").removeClass('d-none');
		$("#record-add-cancel-button").attr('href',"customer-groups.php#"+customergroupfuncs.prev_page_hash);
		$('#msgFrm').removeClass('d-none');
		var coming_from='';
		var options={mode:'editrecord',recordid:recordid,leadtabtext:'Edit Customer Group',coming_from:coming_from}
	...
	openRecord:function(options){
		var self=customergroupfuncs;
		var opts={leadtabtext:'Customer Group Details'};
		$.extend(true,opts,options);

		customergroupfuncs.dep_rowno_max=-1;
		...
		if(otherparams.mode=='editrecord'){
			...
			if(rec_id!=''){
				...
				if(resp[1].can_edit===false){
					...
					location.hash=customergroupfuncs.prev_page_hash;
					return;
				}

				customergroupfuncs.removeEditRestrictions();
				...
				let header_text = 'Edit Customer Group';
				...
				contobj.find("#infoMsg").html('Edit Customer Group <b>' + name_disp +  '</b>');
				customergroupfuncs.setheaderBarText(header_text);

				customergroupfuncs.applyEditRestrictions(resp[1].edit_restricted_fields);
				...
				if(resp[0]==1){
					message="Sorry, the edit window could not be opened (Customer Group ID missing).";
				}else if(resp[0]==2){
					message="Sorry, the edit window could not be opened (Server error).";
				}else if(resp[0]==3){
					message="Sorry, the edit window could not be opened (Invalid customer group ID).";
				}
	...
	validateRecDetails:function(opts){
		...
		if(name == ''){
			errors.push('Customer Group name is required.');
			error_fields.push(name_fld_id);
			$(name_fld_id).addClass("error-field");

		}else if(mode=='edit-rec' && active==''){
			errors.push('Please select a status for the customer group.');
	...
	openAddUserForm:function(e){
		...
		$("#edit-record-button").addClass('d-none');
		$("#rec_list_container").addClass('d-none');
		$("#rec_detail_add_edit_container").removeClass('d-none').find("#panel-heading-text").text('Add Customer Groups').end();
		$('#msgFrm').removeClass('d-none');
			
		$(".back-to-list-button").removeClass('d-none');
		
		$("#rec_detail_add_edit_container").find("#record-save-button>span:eq(0)").html('Add Customer Groups').end().find("#add_edit_mode").val('createrec').end().find("#add_edit_recordid").val('').end().find("#record-add-cancel-button").data('back-to','').attr('href',"customer-groups.php#"+customergroupfuncs.prev_page_hash);
	...
	deleteCustomerGroup:function(ev){
		ev.preventDefault();
		ev.stopPropagation();
		...
		if(confirm(`Really delete the customer group "${groupname}" ?`)){

			let rec_details = {};
			common_js_funcs.callServer({cache:'no-cache',async:false,dataType:'json',type:'post',url:customergroupfuncs.ajax_data_script,params:{mode:'deleteCustomerGroup', rec_id:id},
				successResponseHandler:function(resp,status,xhrobj){
					if(resp.error_code == 0)
						customergroupfuncs.handleDeleteResp(resp);
					else
						alert(resp.message);
				},
	...
	handleDeleteResp:function(resp){
		alert(resp.message);
		customergroupfuncs.refreshList();
	},
	...
	onHashChange:function(e){
		...
		if(customergroupfuncs.curr_page_hash!=customergroupfuncs.prev_page_hash){
			customergroupfuncs.prev_page_hash=customergroupfuncs.curr_page_hash;
		}
		customergroupfuncs.curr_page_hash=hash;
		...
		switch(hash_params.mode.toLowerCase()){
			case 'addrec':
				if(!CAN_ADD)
					window.history.pushState({}, "", CONST_APP_ABSURL+ customergroupfuncs.ajax_data_script +'#'+customergroupfuncs.list_page_hash);
				else{
					$('.alert-success, .alert-danger').addClass('d-none');
					$('#msgFrm').removeClass('d-none');
					customergroupfuncs.openAddUserForm();
				}
				break;
			...
			case 'edit':
				if(!CAN_EDIT)
					...
					if(hash_params.hasOwnProperty('recid') && hash_params.recid!=''){
						customergroupfuncs.openRecordForEditing(hash_params.recid);
			...
			default:
					if(hash_params.mode==='')
						customergroupfuncs.list_page_hash = hash;
					$('.alert-success, .alert-danger').addClass('d-none');
					$('#msgFrm').removeClass('d-none');
					var params={mode:'getList',pno:1, searchdata:"[]", sortdata:JSON.stringify(customergroupfuncs.sortparams), listformat:'html'};
					...
					customergroupfuncs.searchparams=JSON.parse(params['searchdata']);
					customergroupfuncs.sortparams=JSON.parse(params['sortdata']);

					if(customergroupfuncs.sortparams.length==0){
						customergroupfuncs.sortparams.push(customergroupfuncs.default_sort);
						params['sortdata']=JSON.stringify(customergroupfuncs.sortparams);
					}

					if(customergroupfuncs.searchparams.length>0){
							$.each(customergroupfuncs.searchparams, function(idx,data) {
					...
					common_js_funcs.callServer({cache:'no-cache',async:true,dataType:'json',type:'post', url:self.ajax_data_script,params:params,successResponseHandler:customergroupfuncs.showList,successResponseHandlerParams:{self:customergroupfuncs}});
