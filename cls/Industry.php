...
class Industry{
	public static function getList($options=[]){
		...
		$fields_mapper['*']="ind.id as id, ind.industry as industry, ind.active as active";

		$fields_mapper['recordcount']='count(distinct(ind.id))';
		$fields_mapper['id']="ind.id";
		$fields_mapper['industry']="ind.industry";
		$fields_mapper['active']="ind.active";
						...
						switch($filter['type']){
							case 'IN':
								if(is_array($filter['value'])){
									$place_holders=[];
									$k=0;
									foreach($filter['value'] as $industry){
										$k++;
										$place_holders[]=":whr".$field_counter."_industry_{$k}_";
										$str_params_to_bind[":whr".$field_counter."_industry_{$k}_"] = $industry;
									}
									$where_clause[] = $fields_mapper[$filter['field']].' not in('.implode(',',$place_holders).') ';
								}
								break;
							...
							case 'CONTAINS':
								$industry=(is_array($filter['value']))?$filter['value'][0]:$filter['value'];
								$where_clause[] = $fields_mapper[$filter['field']].' like :whr'.$field_counter.'_industry_';
								$str_params_to_bind[':whr'.$field_counter.'_industry_']='%'.$industry.'%';
								break;

							case 'NOT_EQUAL':
								$industry=(is_array($filter['value']))?$filter['value'][0]:$filter['value'];
								$where_clause[] = $fields_mapper[$filter['field']].'!=:whr'.$field_counter.'_industry_';
								$str_params_to_bind[':whr'.$field_counter.'_industry_']=$industry;
								break;
		...
		if(array_key_exists('group_by', $options) && is_array($options['group_by'])){
			foreach ($options['group_by'] as $field) {
				if(preg_match("/^(ind)\./",$fields_mapper[$field]))
		...
		if(array_key_exists('order_by', $options) && is_array($options['order_by'])){
			foreach ($options['order_by'] as $order) {
				if(preg_match("/^(ind)\./",$fields_mapper[$order['field']])){
			...
			if($order_by_clause!='' && !stristr($order_by_clause, 'ind.id')){
		...
		if(!$record_count && $order_by_clause==''){
			...
			$order_by_clause=" ORDER BY ind.industry ASC";

			if(!preg_match("/\s+as\s+industry/",$select_string)){
				$select_string .= ', '.$fields_mapper['industry'].' as industry';
			}
		}
		...
		$sql="SELECT $select_string from `".CONST_TBL_PREFIX."industries` as ind $where_clause_string $group_by_clause $order_by_clause $limit_clause";
	...
	static function assignRevoke(int $mem_id, array $industry_ids=[]){ 
		$str_data = $int_data = [];
		if(empty($mem_id))
			return false;
		...
		try{
			...
			$stmt_obj = PDOConn::query('DELETE from `'.CONST_TBL_PREFIX . 'member_industries` WHERE mem_id=:mem_id', [], $int_data);
			if(empty($stmt_obj)){
				return false;
			}
			if(!empty($industry_ids)){
				$insert_sql = 'INSERT INTO `'.CONST_TBL_PREFIX . 'member_industries`(`mem_id`, `industry_id`) VALUES ';
				$values=array();
	...
	static function assignRevokeFromReg(int $mem_id, array $industry_ids=[]){ 
		$str_data = $int_data = [];
		if(empty($mem_id))
			return false;
		...
		try{
			...
			$stmt_obj = PDOConn::query('DELETE from `'.CONST_TBL_PREFIX . 'member_regs_industries` WHERE mem_id=:mem_id', [], $int_data);
			if(empty($stmt_obj)){
				return false;
			}
			if(!empty($industry_ids)){
				$insert_sql = 'INSERT INTO `'.CONST_TBL_PREFIX . 'member_regs_industries`(`mem_id`, `industry_id`) VALUES ';
				$values=array();
				foreach($industry_ids as $id){
					$key = ":industryid_{$id}";
					$values[]="(:mem_id, $key)";
	...
	static function add(array $industries){ 
		$str_data = [];
		if(empty($industries))
			return false;
		$industry_ids = [];
		try{
			foreach($industries as $industry){
				$insert_sql =  'INSERT INTO `'.CONST_TBL_PREFIX . 'industries` set `industry` = :industry ON DUPLICATE KEY UPDATE `active`="y" ';
				$str_data[':industry'] = $industry;
				$stmt_obj = PDOConn::query($insert_sql, $str_data);
				if($stmt_obj===false)
					return false;
				$industry_ids[] = PDOConn::lastInsertId();
			}
			return $industry_ids;
		}catch(\Exception $e){
			if(!is_a($e, '\PDOStatement'))
	...
	static function update($data, int $id){ 
		...
		if(empty($data) || empty($id))
			return false;
		try{
			$sql =  ' UPDATE `'.CONST_TBL_PREFIX . 'industries` set '; 
			if(!empty($data['industry'])){
				$sql .=' `industry` = :industry '; 
				$str_data[':industry'] = $data['industry'];
			}
	...
	static function delete(array $industry_ids){ 
		$str_data = [];
		if(empty($industry_ids))
			return false;
		try{
			$sql = 'DELETE FROM `'.CONST_TBL_PREFIX . 'industries` WHERE id in';
			$placeholders = [];
			$idx=0;
			foreach($industry_ids as $id){
				$idx++;
