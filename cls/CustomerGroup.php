...
class CustomerGroup {
    public static function getList($options = []) {
        ...
        $fields_mapper['*'] = "cg.id as id, cg.name as name, cg.active as active";

        $fields_mapper['recordcount'] = 'count(distinct(cg.id))';
        $fields_mapper['id'] = "cg.id";
        $fields_mapper['name'] = "cg.name";
        $fields_mapper['active'] = "cg.active";
        ...
        if (array_key_exists('group_by', $options) && is_array($options['group_by'])) {
            foreach ($options['group_by'] as $field) {
                if (preg_match("/^(cg)\./",$fields_mapper[$field])) {
        ...
        if (array_key_exists('order_by', $options) && is_array($options['order_by'])) {
            foreach ($options['order_by'] as $order) {
                if (preg_match("/^(cg)\./",$fields_mapper[$order['field']])) {
            ...
            if ($order_by_clause != '' && !stristr($order_by_clause, 'cg.id')) {
                $order_by_clause .= ', '.$fields_mapper['id'].' DESC ';
            }
        }

        if (!$record_count && $order_by_clause == '') {
            $order_by_clause = " ORDER BY cg.name ASC";
        ...
        $sql = "SELECT $select_string from `".CONST_TBL_PREFIX."customer_groups` as cg $where_clause_string $group_by_clause $order_by_clause $limit_clause";
    ...
    static function add(array $customer_groups) { 
        $str_data = [];
        ...
        try {
            foreach ($customer_groups as $customer_group) {
                $insert_sql = 'INSERT INTO `'.CONST_TBL_PREFIX . 'customer_groups` set `name` = :name ON DUPLICATE KEY UPDATE `active`="y" ';
                $str_data[':name'] = $customer_group;
                $stmt_obj = PDOConn::query($insert_sql, $str_data);
                if ($stmt_obj === false) {
                    return false;
                }
                $customer_group_ids[] = PDOConn::lastInsertId();
            }
            return $customer_group_ids;
        } catch (\Exception $e) {
            $error_details_to_log = [];
            $error_details_to_log['function'] = __METHOD__;
            $error_details_to_log['input'] = $customer_groups;
    ...
    static function update($data, int $id) { 
        ...
        try {
            $sql = ' UPDATE `'.CONST_TBL_PREFIX . 'customer_groups` set '; 
            $flds = [];
            if (!empty($data['name'])) {
                $flds[]= ' `name` = :name '; 
    ...
    static function delete(array $customer_group_ids) { 
        $str_data = $int_data = [];
        if (empty($customer_group_ids)) {
            return false;
        }
        try {
            $sql = 'DELETE FROM `'.CONST_TBL_PREFIX . 'customer_groups` WHERE id in';
            $placeholders = [];
            $idx = 0;
            ...
            $error_details_to_log = [];
            $error_details_to_log['function'] = __METHOD__;
            $error_details_to_log['input'] = $customer_group_ids;
