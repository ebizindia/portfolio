...
class Item {
    ...
    public function getDetails($fields_to_fetch=[]) {
        ...
        $options = [];
        $options['filters'] = [
            ['field'=>'id', 'type'=>'=', 'value'=> $this->item_id]
        ];
    ...
    public static function getList($options=[]) {
        $data = [];
        $fields_mapper = [];
        $fields_mapper['*'] = 'i.id as id, i.name, i.make, i.unit, i.created_on, i.created_by, i.created_from_ip, i.updated_on, i.updated_by, i.updated_from_ip';
        $fields_mapper['recordcount'] = 'count(i.id)';
        $fields_mapper['id'] = 'i.id';
        $fields_mapper['name'] = 'i.name';
        $fields_mapper['make'] = 'i.make';
        $fields_mapper['unit'] = 'i.unit';
        $fields_mapper['created_on'] = 'i.created_on';
        $fields_mapper['created_by'] = 'i.created_by';
        $fields_mapper['created_from_ip'] = 'i.created_from_ip';
        $fields_mapper['updated_on'] = 'i.updated_on';
        $fields_mapper['updated_by'] = 'i.updated_by';
        $fields_mapper['updated_from_ip'] = 'i.updated_from_ip';
        ...
        if(array_key_exists('order_by', $options) && is_array($options['order_by'])) {
            ...
            if($order_by_clause != '' && !stristr($order_by_clause, 'i.id')) {
                $order_by_clause .= ', i.id DESC';
            }
        }
        
        if(!$record_count && $order_by_clause == '') {
            $order_by_clause = " ORDER BY i.name ASC, i.id DESC";
        }
        ...
        $sql = "SELECT $select_string FROM `" . CONST_TBL_PREFIX . "items` as i $where_clause_string $group_by_clause $order_by_clause $limit_clause";
    ...
    public function add(array $data) {
        ...
        $sql = "INSERT INTO `" . CONST_TBL_PREFIX . "items` SET ";
        $values = [];
    ...
    public function update($data) {
        $str_data = $int_data = [];
        if(empty($data) || empty($this->item_id))
            return false;
            
        $int_data[':id'] = $this->item_id;
        $sql = "UPDATE `" . CONST_TBL_PREFIX . "items` SET ";
        $values = [];
        ...
        $error_details_to_log['sql'] = $sql;
        $error_details_to_log['data'] = $data;
        $error_details_to_log['id'] = $this->item_id;
    ...
    public static function delete($ids) {
        ...
        $sql = "DELETE FROM `" . CONST_TBL_PREFIX . "items` WHERE `id` IN (" . implode(',', $place_holders) . ")";
     ...
     * @return bool True if name exists, false if it doesn't
     */
    public static function nameExists($name, $exclude_id = null) {
        $str_params_to_bind = [];
        $int_params_to_bind = [];

        $str_params_to_bind[':name'] = $name;

        $sql = "SELECT id FROM `" . CONST_TBL_PREFIX . "items` WHERE `name` = :name";

        if ($exclude_id !== null) {
            $sql .= " AND `id` != :exclude_id";
            $int_params_to_bind[':exclude_id'] = $exclude_id;
        }
        ...
        try {
            $pdo_stmt_obj = PDOConn::query($sql, $str_params_to_bind, $int_params_to_bind);
            $data = $pdo_stmt_obj->fetch(\PDO::FETCH_ASSOC);
            return ($data !== false);
        } catch(\Exception $e) {
            if(!is_a($e, '\PDOStatement'))
                ErrorHandler::logError($error_details_to_log, $e);
